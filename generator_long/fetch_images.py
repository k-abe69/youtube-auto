...
def fetch_all_images(scene_json_path: Path, script_id: str, start_index: int, batch_size: int):
    ...
    output_dir = Path(f"data_long/stage_5_image/sd_images/{script_id}")
    ...
    for i, parent_id in enumerate(batch):
        # ファイル名決定
        mark_mv_path = Path(f"data_long/stage_2_tag/mark_mv/{script_id}/{parent_id}_mv.txt")
...
if __name__ == "__main__":
    ...
    input_dir = Path("data_long/stage_3_prompt")
    output_dir = Path("data_long/stage_5_image")
